...
export function dfasAreIsomorphic(left, right) {
  const alphabet = [...new Set([...left.alpha, ...right.alpha])].sort();

  if (Object.keys(left.states).length !== Object.keys(right.states).length) return false;

  const mapLeftToRight = new Map();
  const mapRightToLeft = new Map();
  const queue = [[left.startKey, right.startKey]];

  mapLeftToRight.set(left.startKey, right.startKey);
  mapRightToLeft.set(right.startKey, left.startKey);

  while (queue.length) {
    const [leftKey, rightKey] = queue.shift();

    if (left.acceptKeys.has(leftKey) !== right.acceptKeys.has(rightKey)) return false;

    for (const symbol of alphabet) {
      const nextLeft = left.trans[leftKey]?.[symbol];
      const nextRight = right.trans[rightKey]?.[symbol];
